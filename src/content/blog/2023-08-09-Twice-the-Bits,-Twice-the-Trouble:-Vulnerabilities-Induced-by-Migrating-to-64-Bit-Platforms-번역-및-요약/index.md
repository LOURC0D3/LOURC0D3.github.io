...
date: 2023-08-09
tags: ["pwnable", "Integer issues", "Computer Science", "Research", "Paper"]
---
- 원문
...
_Figure 1: Integer truncation → buffer overflow_
...
_Figure 2: Integer overflow → buffer overflow_
...
_Figure 3: Sign-extension → buffer overflow_
...
_Figure 4: Signed comparison → buffer overflow_
...
_Figure 5: Example of a 64-bit migration vulnerability caused by incorrect pointer differences_
...
_Figure 6: A check to avoid buffer overflows on 32-bit
systems that is ineffective on LP64 platforms_
...
_Figure 7: Buffer overflow resulting from an integer
overflow due to larger strings on 64-bit platforms_
...
_Figure 8: Buffer overflow caused by the common pattern of assigning the result of strlen to an int_
...
- 64bit 플랫폼에서는 문자열의 길이가 INT_MAX 보다 클 수 있으므로 반환되는 데이터가 snprintf의 반환 자료형인 int를 초과할 수 있다.
    - 해당 경우에 대해 C99 표준은 -1을 반환하도록 정의하였다.
        ...
        _예 : glibc-2.29/stdio-common/vfprintf-internal.c:1242 - vfprintf_
...
_Figure 9: Stack-corruption vulnerability on 64-bit systems due to unexpected behavior of snprintf_
...
_Figure 10: Buffer overflow for files larger than UINT_MAX caused by unexpected return value of ftell._
